...
#   two endpoints already present. See asymptote_estimation.R -- the asymptote is
...
if (length(miss)) stop("missing inputs:\n  ", paste(miss, collapse="\n  "),
  "\nrun inventory_build.R, predict_tree_flux_current.R, predict_soil_surface.R, budget_canonical.R first")
...
# One allometry, from geometry.R. This was a copy-pasted duplicate that retyped
...
# predict_tree_flux_current.R now exports the calibrated profile it already
...
# wai_bottomup_and_rf_interactions.R on the PRE-REBUILD inventory (7,010 stems,
...
WAI_BU <- local({
  ...
  if (!file.exists(f))
    stop("missing ", f, " -- run: Rscript code/06_upscale/wai_bottomup_and_rf_interactions.R")
...
#                           6.899 vs constant 0.425), rf_height_extrapolation.R
...
# ---- export the SHAPES themselves, for fig_scaling_profiles.R ------------
