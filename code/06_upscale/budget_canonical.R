...
#!/usr/bin/env Rscript
# ==============================================================================
# budget_canonical.R  --  THE canonical budget.
# ------------------------------------------------------------------------------
...
#
#   inventory      inventory_build.R        -> inventory_stems.csv
#   tree, per stem predict_tree_flux_current.R -> tree_flux_predictions.csv
#   tree, monthly  predict_tree_flux_current.R -> tree_monthly_stand.csv
#   soil, surface  predict_soil_surface.R   -> soil_surface_{monthly,annual}.csv
#   geometry       geometry.R               -> STAND_AREA_M2
#
...
#     square minus the 7 uncensused 20 m quadrats. See geometry.R, which is
...
if (length(miss)) stop("missing inputs:\n  ", paste(miss, collapse = "\n  "),
  "\nrun, in order: inventory_build.R, predict_tree_flux_current.R, predict_soil_surface.R")
...
M <- data.frame(month = 1:12) %>%
  ...
  mutate(tree_bh_nmol_m2_s = tree_bh_nmol_m2_s,
         ...
         # only 0.45%, but fig09_budget.R plots this series in panel (c) under the
...
# reach for the flattering one by default. NA if rf_grouped_cv.R has not run.
GCV <- local({
  f <- "outputs/data/rf_grouped_cv.csv"
  if (!file.exists(f)) { cat("NOTE: rf_grouped_cv.csv absent -- run rf_grouped_cv.R\n")
