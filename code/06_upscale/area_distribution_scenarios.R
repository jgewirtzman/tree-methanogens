...
#     the bytag coordinate rescue and the DBH unit fix (inventory_build.R);
#   * an fx() stack of species-specific DBH "repairs" that inventory_build.R
...
#   * the species-ladder bug (see species_levels.R).
# It now reads the same products scaling_full_grid.R reads, so the two cannot
...
if (any(!file.exists(NEED)))
  stop("missing ", paste(NEED[!file.exists(NEED)], collapse=", "),
       " -- run inventory_build.R and predict_tree_flux_current.R first")
...
INV$H <- stem_height_m(INV$dbh, INV$species)
