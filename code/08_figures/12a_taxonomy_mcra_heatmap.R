...
#   abundance) + any additional families significantly associated with mcrA
#   (FDR < 0.01) that are not already in the top 20. Families are
#   hierarchically clustered; samples are ordered by mcrA abundance.
#
...
#   4. Filter to wood samples with mcrA data and 16S >= 100
#
# Inputs:
...
# Load PICRUSt metadata for mcrA + 16S qPCR values used in LMER
...
# ==============================================================================
# STEP 6: Run LMER association tests
# ==============================================================================
...
pvals <- run_lmer_associations(percent_filt, wood)
...
# Families significantly associated with mcrA (FDR < 0.05)
...
ann_col <- wood[ordered_samples, c("compartment", "log10_mcra"), drop = FALSE]
colnames(ann_col) <- c("Compartment", "log10(mcrA)")
...
ann_colors <- list(
  ...
  `mcrA assoc.` = c("FDR < 0.01" = "#b71c1c", "FDR < 0.05" = "#ef9a9a", "NS" = "#f0f0f0")
)
...
png("outputs/figures/original/supplementary/figS6_taxonomy_mcra_heatmap.png",
