...
# ==============================================================================
# Purpose: Analyzes variable importance via partial dependence plots for tree
#   and soil RF models.
#
...
#
# Outputs:
#   - partial dependence PNGs (to outputs/figures/)
# ==============================================================================
...
for(feature in tree_key_features) {
  if(feature %in% colnames(X_tree)) {
    ...
    pd_result$yhat_original <- (pd_result$yhat)
    pd_result$yhat_nmol <- pd_result$yhat_original
...
png("../../outputs/figures/PD_TREE_MODEL.png", width = 12, height = 4 * n_rows)
...
png("../../outputs/figures/PD_SOIL_MODEL.png", width = 12, height = 4 * n_rows_soil)
...
if(all(c("soil_moisture_at_tree", "air_temp_C_mean") %in% colnames(X_tree))) {
  ...
  pd_2d_tree$yhat_nmol <- (pd_2d_tree$yhat)
  ...
  ggsave("../../outputs/figures/PD_2D_TREE_MOISTURE_TEMP.png", p_tree_2d, width = 10, height = 8)
}
...
if(all(c("soil_moisture_at_site", "soil_temp_C_mean") %in% colnames(X_soil))) {
  ...
  pd_2d_soil$yhat_nmol <- (pd_2d_soil$yhat)
  ...
  ggsave("../../outputs/figures/PD_2D_SOIL_MOISTURE_TEMP.png", p_soil_2d, width = 10, height = 8)
}
...
if(all(c("month_sin", "month_cos") %in% colnames(X_tree))) {
  ...
  for(i in 1:12) {
    ...
    tree_month_pd$yhat_nmol[i] <- (pred)
  }
  ...
  ggsave("../../outputs/figures/PD_TREE_MONTH_EFFECT.png", p_tree_month, width = 10, height = 6)
}
...
if(all(c("month_sin", "month_cos") %in% colnames(X_soil))) {
  ...
  for(i in 1:12) {
    ...
    soil_month_pd$yhat_nmol[i] <- (pred)
  }
  ...
  ggsave("../../outputs/figures/PD_SOIL_MONTH_EFFECT.png", p_soil_month, width = 10, height = 6)
}
...
if("soil_moisture_at_tree" %in% colnames(X_tree)) {
  ...
  ice_tree_moisture$yhat_nmol <- (ice_tree_moisture$yhat)
  ...
  ggsave("../../outputs/figures/ICE_TREE_MOISTURE.png", p_ice, width = 10, height = 7)
}
