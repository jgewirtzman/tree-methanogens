#!/usr/bin/env Rscript
# ==============================================================================
# inventory_build.R  --  THE stem inventory, rebuilt from raw.
# ------------------------------------------------------------------------------
# Run from the repo root:  Rscript code/01_import/inventory_build.R
...
# predict_tree_flux_current.R read as its own input and overwrote as its own
# output, and that budget_canonical.R also wrote with a different stem set.
...
local({
  ...
  if (!identical(as.integer(got), as.integer(sort(UNCENSUSED_QUADRATS)))) {
    ...
    stop("uncensused quadrats have drifted from UNCENSUSED_QUADRATS in geometry.R")
  }
  cat("  matches UNCENSUSED_QUADRATS in geometry.R\n")
...
wr("INVENTORY PROVENANCE\ngenerated by code/01_import/inventory_build.R\n")
