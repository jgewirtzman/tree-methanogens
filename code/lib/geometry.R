#!/usr/bin/env Rscript
# ==============================================================================
# geometry.R  --  THE stand geometry. Source this; never redefine plot area.
# ------------------------------------------------------------------------------
...
geo_transforms <- function(rdata = "data/processed/integrated/rf_workflow_input_data_with_2023.RData") {
  # Callable from the repo root or from code/05_model/, whose scripts use ../../
...
local({
  ...
  if (abs(a - STAND_AREA_M2) > 1e-6)
    stop(sprintf("geometry.R: ring area %.1f != STAND_AREA_M2 %.1f", a, STAND_AREA_M2))
})
