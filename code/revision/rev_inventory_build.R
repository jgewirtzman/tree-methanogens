...
# ------------------------------------------------------------------------------
# Run from the repo root:  Rscript code/revision/rev_inventory_build.R
...
wr("INVENTORY PROVENANCE\ngenerated by code/revision/rev_inventory_build.R\n")
wr("built: %s\nR: %s\n\n", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), R.version.string)
wr("RAW INPUTS (md5)\n")
for (s in srcs) wr("  %-58s %s\n", basename(s), md5[[s]])
wr("\nGEOMETRY\n  stand = %d x %d m minus notch PX %d-%d, PY 0-%d\n",
   PLOT_SIDE_M, PLOT_SIDE_M, NOTCH_PX_MIN, PLOT_SIDE_M, NOTCH_PY_MAX)
wr("  stand area %d m2 (%.2f ha); nominal %d m2; notch %d m2\n",
   STAND_AREA_M2, STAND_AREA_M2/1e4, PLOT_AREA_M2, NOTCH_AREA_M2)
...
wr("  excluded: %d in notch, %d outside square\n",
