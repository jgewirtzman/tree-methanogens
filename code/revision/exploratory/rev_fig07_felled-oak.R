# ==============================================================================
# REVISION — Felled Oak Vertical Profiles (Figure 7), FINAL
# ==============================================================================
# Copy of code/06_figures/09_felled_oak_profiles.R with two in-place fixes only,
# preserving ALL original styling / legend positions / dimensions:
#   * Panel b: mcrA in copies/g DRY wood (was copies/uL) — R2 Fig 7b.
#   * Panel c: CH4 flux axis given its unit (nmol m-2 s-1) — R2 Fig 7c.
# NEW file; edits nothing existing. Output: outputs/revision/fig7_felled_oak_final.png
#
...
# --- FIX (R2 Fig 7b): convert copies/uL -> copies/g DRY wood ------------------
# copies/g = conc(copies/uL) * elution_uL / mass_g; cores freeze-dried => dry wt.
...
bo_mass <- readr::read_csv("data/processed/molecular/black_oak/bo_extraction_mass.csv", show_col_types = FALSE)
mcra <- mcra %>%
  left_join(bo_mass %>% transmute(`Sample ID`, mass_mg = `Sample Mass Added to Tube (mg)`),
            by = "Sample ID") %>%
  mutate(mcrA_copies_g = Conc_copies_uL * ELUTION_UL / (mass_mg / 1000) * DILUTION_10X)
...
p2 <- ggplot(mcra_clean, aes(y = mcrA_copies_g, x = Height_cm / 100)) +
  ...
  coord_flip() +
  ylab(expression(italic(mcrA)~"(copies g"^-1*" dry)")) +
  xlab("") +
  scale_x_continuous(breaks = height_breaks, minor_breaks = NULL) +
...
ggsave("outputs/revision/fig7_felled_oak_final.png",
       fig7, width = 12, height = 5.1, dpi = 300)
cat("Wrote outputs/revision/fig7_felled_oak_final.png (copies/g dry; flux unit; DILUTION_10X =", DILUTION_10X, ")\n")
