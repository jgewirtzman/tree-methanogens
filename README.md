...
To reproduce analyses, download the Zenodo archive and extract into `data/`. The directory structure is preserved in git via `.gitkeep` files so all paths are pre-configured. Processed data can be regenerated by running the pipeline scripts in order. All figures can be regenerated via `Rscript code/generate_all_figures.R`.
