...
│   │   └── soil/               # Soil flux data
│   ├── compiled/               # Clean, analysis-ready datasets for Zenodo
│   └── deprecated/             # Removed duplicates, scratch files, non-data files
...
- **`data/compiled/`** — 12 analysis-ready CSV datasets with clear column names and documented units, suitable for reanalysis and meta-analysis. See `data/compiled/README_data.md` for full column dictionaries.
...
To reproduce analyses, download the data archive and extract its contents into `data/`. The directory structure is preserved in git via `.gitkeep` files so all paths are pre-configured. Processed data in `data/processed/` can be regenerated by running the pipeline scripts in order.
