# Complete click.peak2 workflow for methane flux measurements
library(goFlux)
library(dplyr)
library(purrr)
...
cat("\n=== LOADING AUXFILE ===\n")
auxfile <- read.csv("auxfile_goFlux_with_weather_formatted_datetime.csv")
...
manID.data <- click.peak2(
  ...
  save.plots = "methane_manual_ID_batch1"  # Save plots as PDF
)
...
if(length(ow.data) > batch_size) {
  ...
  if(tolower(continue_response) == "y") {
    ...
    manID.data.2 <- click.peak2(
      ...
      save.plots = "methane_manual_ID_batch2"
    )
