...
plot_data_top <- plot_data_top %>%
  left_join(tree_counts %>% dplyr::select(species, species_label), by = "species") %>%
...
# Top panel: Boxplots with breaks calculated from original data
p_top <- ggplot(plot_data_top, aes(x = factor(height_m), y = CH4_best.flux)) +
  ...
  scale_y_continuous(
    breaks = function(x) {
      # Get the current panel's species label
      current_panel <- unique(plot_data_top$species_label[plot_data_top$CH4_best.flux >= min(x, na.rm = TRUE) & 
                                                            plot_data_top$CH4_best.flux <= max(x, na.rm = TRUE)])
      
      # If we can identify the panel, use our pre-calculated breaks
      if(length(current_panel) == 1 && current_panel %in% names(breaks_lookup)) {
        return(breaks_lookup[[current_panel]])
      } else {
        # Fallback to original method if panel identification fails
...
# Prepare data for bottom panel (coefficients) - unchanged
analysis_data <- plot_data_top %>%
  mutate(tree_unique = paste(plot, tree_id, sep = "_"))

# Function to test height effect for each species - unchanged
...
# Bottom panel: Coefficient plot with solid/hollow points - unchanged
...
# Create individual plots with tighter spacing
