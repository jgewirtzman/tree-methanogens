...
#Modified by Jon Gewirtzman over 2022
...
#This must include a start and end time and a unique ID for each flux observation; it will use those keys to calculate each flux
...
#Final dataframe thus has all flux IDs labeled by what flux calculation they should be part of
...
#The loop will break if there are any issues where start times/end times are wrong-- e.g. if a decimal is wrong and there's a negative time length et.
...
#some manual corrections for my field notes... ignore
str(output2)
