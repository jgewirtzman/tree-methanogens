...
LGR_fn <- read.table("Gewirtzman_datasheets.csv", fill=TRUE,header=TRUE,sep=",",na.strings=c("NA","#N/A!","#N/A","#NA!"))    
...
LGR_fn$Flux.Start.Time<-as.character(LGR_fn$Flux.Start.Time)
LGR_fn$Flux.End.Time<-as.character(LGR_fn$Flux.End.Time)
...
LGR_fn$Real_time
Real_time<-lubridate::hms(LGR_fn$Flux.End.Time)
