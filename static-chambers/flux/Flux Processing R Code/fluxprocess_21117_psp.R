...
ggplot(data=output2, aes(x=Measurement.height, y=CH4_lin_flux, fill=as.factor(Measurement.height)))+
  ...
  #geom_smooth()+
  geom_boxplot()+
  facet_wrap(vars(Tree_sp), scales="free_x")+
  #ylim(-2, 10)+
  coord_flip()+
...
outputz<-output2[which(output2$Tree_sp=="BEAL" | output2$Tree_sp=="FRAM"),]
output3<-output2[which(output2$Tree_sp!="BEAL"),]
...
p6<-
  ggplot(data=output2, aes(x=Measurement.height, y=CH4_lin_flux))+
  #geom_smooth()+
  geom_jitter(alpha=0.5)+
  geom_boxplot(aes(color=as.factor(Measurement.height)), alpha=0.7)+
  ...
  ylim(-.01, 0.5)
ggsave("fig6.png", p6, height=5, width=8)

pz<-ggplot(data=outputz, aes(x=Measurement.height, y=CH4_lin_flux, 
...
p19<-
ggplot(data=output2, aes(x=wood, y=log(CH4_lin_flux)))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(aes(color=Tree_sp))+xlab("")+ylab("Log CH4 Flux, nmol m-2 s-1")+theme_cowplot()
...
ef<-read.csv("/Users/jongewirtzman/Dropbox (YSE)/1 Eastern US internal CH4/From Covey 9_21/easternforest_clean_wmeta.csv")
