...
#LGR_fn$Flux.Start.Time<-as.character(LGR_fn$Corrected.Start.Time)
#LGR_fn$Flux.End.Time<-as.character(LGR_fn$Corrected.End.Time)

#for(i in 1:nrow(LGR_fn)){
#  if(nchar(LGR_fn$Flux.Start.Time[i])==5){
#    LGR_fn$Flux.Start.Time[i]=paste("0", LGR_fn$Flux.Start.Time[i], sep="")
#  }
#}
#LGR_fn=LGR_fn[which(nchar(LGR_fn$Flux.Start.Time)==6),]

#LGR_fn$Real_time
#Real_time<-lubridate::hms(LGR_fn$HMS_Start)
#strptime("010101","%H%M%S")

LGR_fn$Date_full<-paste("20", LGR_fn$Date, sep="")
LGR_fn$date.parsed<-ymd(LGR_fn$Date_full)

LGR_fn$start.time.final<-ymd_hms(paste(LGR_fn$date.parsed, LGR_fn$HMS_Start))

LGR_fn$end.time.final<-ymd_hms(paste(LGR_fn$date.parsed, LGR_fn$HMS_End))
...
fuzzy_left_join(LGR_data, LGR_fn,
                by=c("date_time"="start.time.final", "date_time"="end.time.final"),
                match_fun=list(`>=`, `<=`))
...
#LGR_data.subset.datetime<-as_datetime(LGR_data.subset$date_time)
#dates<-LGR_data.subset.datetime
...
fuzzy_left_join(LGR_fn.labels, LGR_data.subset.datetime,
                by=c("date_time"="start.time.final", "date_time"="end.time.final"),
                match_fun=list(`>=`, `<=`))

library(sqldf)

result = sqldf("select * from lgrds
                left join lgrfn
                on lgrds.date_time between lgrfn.start and lgrfn.end")
